...
pjk = [[3, 4, 3, 4, 4, 1000], [5, 1000, 5, 4, 1000, 4], [3, 4, 6, 1000, 1000, 1000], [2, 4, 1000, 1000, 4, 1000],
       [1, 3, 3, 2, 1000, 1000], [1, 3, 1000, 1000, 2, 1000], [
           2, 2, 2, 1000, 1000, 2], [1, 1000, 1, 1000, 2, 1000],
...
def fitness(individual, data):
    ...
    for machine2 in range(1, data[2]+1):
        ...
        for op4 in range(0, len(operations2)):
            ...
            end_reference = individual[operations2[op4]
                                       * 2]+pjk[operations2[op4]][machine2-1]
            for op5 in range(0, len(operations2)):
                if op5 != op4:
                    s = individual[operations2[op5]*2]
                    c = individual[operations2[op5]*2] + \
                        pjk[operations2[op5]][machine2-1]
...
def genetic_algorithm_scheduling(data, counter, pop_size=100, num_generations=500):
    start_time = time()
    ga = pyeasyga.GeneticAlgorithm(data, maximise_fitness=False, population_size=pop_size,
                                   generations=num_generations, mutation_probability=0.3)  # initialization of the algorithm
